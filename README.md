...
You don't need this to publish — the Actions workflow builds on every push and the live URL is your real preview. But local preview is faster (~1s rebuild on save) and useful for CSS work.

**Requirements:** Ruby 3.1 or newer. macOS's built-in Ruby is too old. The cleanest path:
...
(On Intel Macs swap `/opt/homebrew` for `/usr/local`.)

Drafts in `_drafts/` show up in local preview if you pass `--drafts`:
```bash
...
This site uses **modern Jekyll 4 + a GitHub Actions workflow** rather than the legacy `github-pages` gem. The workflow lives at `.github/workflows/pages.yml`. Every push to `main` builds the site on Ubuntu (Ruby 3.3, Jekyll 4.3) and publishes the result.

### One-time setup

1. **Create a GitHub repo.** For a site at `https://<you>.github.io`, name the repo exactly `<you>.github.io`. For a project site at `https://<you>.github.io/<repo>`, name it whatever you like (e.g. `blog`).
2. **Push this folder up.**
   ...
   ```
3. **Switch Pages to use Actions.** On GitHub: *Settings → Pages → Build and deployment → Source: **GitHub Actions***. (This is the key step — selecting "Actions" instead of "Deploy from a branch" is what frees you from the `github-pages` gem's ancient version pins.)
...
After the first push, watch the *Actions* tab — you'll see the **Deploy site to Pages** workflow running. First build takes ~2 minutes (it's installing gems). Subsequent builds are ~30 seconds because gems are cached.

When it's green, your site is live at the URL shown on the Pages settings page.

### Why this is better than "Deploy from a branch"

- You're on **Jekyll 4** (current), not Jekyll 3.9 (frozen since 2020).
- You can use **any Jekyll plugin**, not just GitHub's allow-list.
- Your **local Ruby version doesn't matter** — CI runs Ruby 3.3 regardless of what you have installed.
- **Build logs are visible** in the Actions tab when something breaks.

### Custom domain (optional)

Add a file named `CNAME` at the repo root containing only your domain (e.g. `yourname.com`), then point the domain's DNS at GitHub Pages per [their docs](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site). The Actions workflow will pick it up automatically.
...
- **Pages settings still shows "Deploy from a branch".** The workflow won't deploy until you switch *Settings → Pages → Source* to **GitHub Actions**.
- **Actions workflow fails on first run with a "Pages site not enabled" error.** Set the Source to GitHub Actions first, then re-run the workflow from the Actions tab.
- **Local preview throws "cannot load such file -- csv" (or `logger`, `base64` etc.).** Your Ruby is 3.4+ and a stdlib gem is missing. The Gemfile already includes the common ones; if a new one comes up, add it the same way and run `bundle install`.
- **`bundle install` is slow or stuck.** Delete `Gemfile.lock` and try again — it's safe to regenerate.
